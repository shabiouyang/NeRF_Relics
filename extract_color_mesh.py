...
if __name__ == "__main__":
    ...
    xyz_ = paddle.FloatTensor(np.stack(np.meshgrid(x, y, z), -1).reshape(-1, 3)).cuda()
    dir_ = paddle.zeros_like(xyz_).cuda()
    ...
    with paddle.no_grad():
        ...
        for i in tqdm(range(0, B, args.chunk)):
            ...
            xyzdir_embedded = paddle.concat([xyz_embedded, dir_embedded], 1)
            out_chunks += [nerf_fine(xyzdir_embedded)]
        rgbsigma = paddle.concat(out_chunks, 0)
    ...
    if args.use_vertex_normal:  ## use normal vector method as suggested by the author.
        ...
        rays_d = paddle.FloatTensor(np.asarray(mesh.vertex_normals))
        near = dataset.bounds.min() * paddle.ones_like(rays_d[:, :1])
        far = dataset.bounds.max() * paddle.ones_like(rays_d[:, :1])
        rays_o = paddle.FloatTensor(vertices_) - rays_d * near * args.near_t
        ...
        results = f([nerf_coarse, nerf_fine], embeddings,
                    paddle.concat([rays_o, rays_d, near, far], 1).cuda(),
                    args.N_samples,
                    args.N_importance,
                    args.chunk,
                    dataset.white_back)
        ...
        for idx in tqdm(range(len(dataset.image_paths))):
            ...
            rays_o = paddle.FloatTensor(dataset.poses[idx][:, -1]).expand(N_vertices, 3)
            ## ray's direction is the vector pointing from camera origin to the vertices
            rays_d = paddle.FloatTensor(vertices_) - rays_o  # (N_vertices, 3)
            rays_d = rays_d / paddle.norm(rays_d, dim=-1, keepdim=True)
            near = dataset.bounds.min() * paddle.ones_like(rays_o[:, :1])
            ...
            far = paddle.FloatTensor(depth) * paddle.ones_like(rays_o[:, :1])
