...
# scheduler
import paddle
from paddle.optimizer.lr import CosineAnnealingDecay, MultiStepDecay, LambdaDecay
...
def get_optimizer(hparams, models):
    ...
    if hparams.optimizer == 'sgd':
        optimizer = SGD(parameters, learning_rate=hparams.lr,
        ...
        optimizer = RAdam(parameters, lr=5e-4, eps=eps,
                          weight_decay=hparams.weight_decay)
...
def get_scheduler(hparams, optimizer):
    ...
    if hparams.lr_scheduler == 'steplr':
        scheduler = MultiStepDecay(learning_rate=5e-4, milestones=hparams.decay_step,
                                gamma=hparams.decay_gamma)
    elif hparams.lr_scheduler == 'cosine':
        scheduler = CosineAnnealingDecay(optimizer, T_max=hparams.num_epochs, eta_min=eps)
    elif hparams.lr_scheduler == 'poly':
        scheduler = LambdaDecay(optimizer,
...
def extract_model_state_dict(ckpt_path, model_name='model', prefixes_to_ignore=[]):
    checkpoint = paddle.load(ckpt_path, map_location=paddle.device('cpu'))
    checkpoint_ = {}
